...
from src.preprocessing.to_bio_subarguments import convert_sample_to_bio_subarguments
from src.utils.subargument_labeling import (
    SUBARGUMENT_LABEL_LIST as LABEL_LIST,
    SUBARGUMENT_LABEL2ID as LABEL2ID,
...
def main():
    ...
    if output_path is None:
        output_path = f"outputs/predictions/seq_subarguments_{args.split}.jsonl"

    raw_data = load_jsonl(data_path)
    processed_data = [convert_sample_to_bio_subarguments(sample) for sample in raw_data]
    ...
    timing = {
        "method": "sequence_labeling_subarguments",
        "split": args.split,
    ...
    timing_output_path = Path(
        f"outputs/timing/seq_subarguments_{args.split}_inference_timing.json"
    )
