...
from src.preprocessing.to_extractive_qa import (
    MAIN_ARGUMENT_LABELS,
    MAIN_ARGUMENT_QUESTIONS,
    extract_gold_main_argument_spans,
...
def get_trigger_predictions_path(config, split):
    return config["trigger_predictions"][f"{split}_path"]
...
def main():
    ...
    parser.add_argument("--trigger_predictions_path", default=None)
    ...
    trigger_predictions_path = (
        args.trigger_predictions_path
        or get_trigger_predictions_path(config, args.split)
    )
    ...
    if output_path is None:
        output_path = f"outputs/predictions/extractive_qa_main_arguments_{args.split}.jsonl"

    max_length = config["training"].get("max_length", 384)
    doc_stride = config["training"].get("doc_stride", 128)
    max_answer_length = config["training"].get("max_answer_length", 60)

    n_best_size = config.get("prediction", {}).get("n_best_size", 50)
    max_spans_per_argument = config.get("prediction", {}).get(
        "max_spans_per_argument",
        1,
    )
    ...
    if len(raw_samples) != len(trigger_predictions):
        raise ValueError(
            f"Raw samples and trigger predictions length mismatch: "
            f"{len(raw_samples)} vs {len(trigger_predictions)}"
        )
    ...
    for record_index, (sample, trigger_record) in enumerate(
        ...
        gold_spans = extract_gold_main_argument_spans(sample)
        pred_spans = []

        predicted_triggers = trigger_record.get("pred_spans", [])

        for pred_trigger in predicted_triggers:
            ...
            for argument_label in MAIN_ARGUMENT_LABELS:
                ...
                answers = predict_n_best_answers_or_no_answer(
                    ...
                    max_spans=max_spans_per_argument,
                    ...
                    pred_spans.append(
                        {
                            ...
                            "score": answer["score"],
                            "source_trigger": {
                                "start": pred_trigger.get("start"),
                                "end": pred_trigger.get("end"),
                                "text": trigger_text,
                                "event_type": pred_trigger.get("event_type"),
                            },
                        }
                    )
    ...
    timing = {
        "method": "extractive_qa_main_arguments_pipeline",
        "split": args.split,
        ...
        "trigger_predictions_path": trigger_predictions_path,
        "device": str(device),
    }

    timing_output_path = Path(
        f"outputs/timing/extractive_qa_main_arguments_{args.split}_inference_timing.json"
    )
